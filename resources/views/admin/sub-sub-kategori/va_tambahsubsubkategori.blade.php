...
@section('content')
    <h2 class="ms-2 my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">Tambah Sub Sub Kategori</h2>
    <div class="px-4 py-3 shadow-md dark:bg-gray-800 height">
        @error('sub-kategori')
            <div class="alert alert-sm alert-danger">{{ $message }}</div>
        @enderror
        <form action="/admin/tambah-sub-sub-kategori" method="post">
            @csrf
            ...
            <label class="block text-sm my-3">
                <span class="mb-2 text-gray-700 dark:text-gray-400 d-block">Pilih Sub Kategori <span
                        class="text-danger">*</span></span>
                <select name="option"
            ...
            <button type="submit" class="btn btn-primary mt-4">Tambah Sub-Sub-Kategori</button>
        </form>
