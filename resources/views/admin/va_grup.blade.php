                                            ...
                                            <tr class="text-gray-700 dark:text-gray-400" id="grup_{{ $grup->id_grup }}">
                                                <td class="py-3 text-center">
                                                    <input type="checkbox" name="ids"
                                                        class="checkbox_ids form-check-input p-2" id=""
                                                        value="{{ $grup->id_grup }}">
                                                </td>
                                                <td class="px-4 py-3">
                                                ...
                                                <td class="px-4 py-3">
                                                    <div class="flex items-center space-x-4 text-sm">
                                                        ...
                                                        <button type="button"
                                                            ...
                                                            data-bs-target="#modalEdit{{ $grup->id }}">
                                                            <i class="bi bi-send-exclamation fs-5"></i>
                                                        </button>

                                                        <!-- Modal Kirim Pesan-->
                                                        <div class="modal fade" id="modalEdit{{ $grup->id }}"
                                                            tabindex="-1" aria-hidden="true">
                                                            <div class="modal-dialog">
                                                                <form
                                                                    action="{{ route('bot.grup.sendmessage') }}?grup={{ $grup->id_grup }}"
                                                                    method="post">
                                                                    @csrf
                                                                    @method('POST')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#sendMessageAllSelected').click(function(e) {
                ...
                if (all_ids.length === 0) {
                    alert('Pilih setidaknya satu grup untuk megirim pesan.');
                    return;
                }
        ...
        <div class="modal fade" id="sendMessageModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form action="{{ route('bot.grup.sendmessage') }}?grup=${id_group}" method="post">
                    @method('POST')
                    @csrf
                    <div class="modal-content">
                        <div class="modal-header">
