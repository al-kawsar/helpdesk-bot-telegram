        ...
        <div class="container grid px-6 mx-auto">
            ...
            <div class="d-flex my-2">
                ...
                <button class="btn btn-primary tambah-kategori ms-auto" type="button" data-bs-toggle="modal"
                    data-bs-target="#modalBot">Tambah
                    {{ $teks }}</button>
            </div>

            <!-- Modal Tambah Bot -->
            <div class="modal fade" id="modalBot" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <form action="{{ route('bot.botsettings.add') }}" method="post">
                        @csrf
                            ...
                            <div class="modal-body">
                                <div class="px-4 py-3 dark:bg-gray-800 height">
                                    ...
                                    <label class="block text-sm">
                                        <span class="text-gray-700 dark:text-gray-400">API KEY BOT<span
                                                class="text-danger">*</span></span>
                                        <input
                                            class=" block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                                            name="keyBot" autocomplete="off" />
                                    </label>
                ...
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap table table-hover">
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @if (request('search') && $bots->isEmpty())
                                <tr>
                            ...
                            @elseif($bots->isEmpty())
                                <tr>
                                    <td colspan="7" class="text-center fs-1 py-5 fw-bold">Tidak Ada Bot</td>
                                </tr>
                            @else
                                @foreach ($bots as $number => $bot)
                                    <tr class="text-gray-700 dark:text-gray-400" id="modal_ids{{ $bot->id }}">
                                        <td class="py-3 text-center">
                                            <input type="checkbox" name="ids" class="checkbox_ids form-check-input p-2"
                                                id="" value="{{ $bot->id }}">
                                        </td>
                                        <td class="px-4 py-3">
                                            {{ $number + $bots->firstItem() }}
                                        </td>
                                        ...
                                        <td class="px-4 py-3 text-sm">
                                            <span class="badge {{ $bot->status == '0' ? 'bg-danger' : 'bg-success' }} p-2">
                                                {{ $bot->status !== '0' ? 'active' : 'not active' }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-3 text-sm">
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center space-x-4 text-sm">
                                                ...
                                                <button type="button"
                                                    ...
                                                    data-bs-toggle="modal" data-bs-target="#modalEdit{{ $bot->id }}">
                                                ...
                                                <div class="modal fade" id="modalEdit{{ $bot->id }}" tabindex="-1"
                                                    aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <form action="{{ route('bot.botsettings.update', $bot->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('PUT')
                                                            <div class="modal-content">
...
try {
                                                                                    // $apiKey = $bot->apikey;
                                                                                    $apikey = Crypt::decrypt($bot->apikey);
                                                                                    echo $apikey;
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        ...
                                                        <li><a class="dropdown-item py-2">
                                                                <button class="" type="button"
                                                                    ...
                                                                    data-bs-target="#modalSetWebhook{{ $bot->id }}">SetWebhook</button>
                                                            </a></li>
                                                        <li>
                                                            <form action="{{ route('bot.webhooksettings.delete',$bot->id) }}" method="POST" class="dropdown-item py-2">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" name="deleteWebhook">Delete Webhook</button>
                                                            </form>
                                                        </li>
                                                        <li><a class="dropdown-item py-2" href="#">Cek Status
                                                                Bot</a></li>
                                                    </ul>
                                                </div>


                                                <div class="modal fade" id="modalSetWebhook{{ $bot->id }}"
                                                    tabindex="-1" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <form action="{{ route('bot.webhooksettings', $bot->id) }}"
                                                            method="post">
                                                            @method('POST')
                                                            @csrf
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body">
                                                                    ...
                                                                    <div class="px-4 py-3 dark:bg-gray-800 height">
                                                                        <label class="block text-sm">
                                                                            <span
                                                                                class="text-gray-700 dark:text-gray-400">Webhook
                                                                                URL (wajib https)<span
                                                                                    class="text-danger">*</span></span>
                                                                            <input
                                                                                class=" block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                                                                ...
                                                                <div class="modal-footer">
                                                                    <button type="button" class="btn btn-danger"
                                                                        data-bs-dismiss="modal">Batal</button>
                                                                    <button type="submit"
                                                                        class="btn btn-success">Setwebhook</button>
                                                                </div>
                                                            </div>
                                                        </form>
                                                    </div>
                ...
                if (all_ids.length >= 1) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed == true) {
                            $.ajax({
                                url: "{{ route('bot.botsettings.selected') }}",
                                type: "DELETE",
                                data: {
                                    ids: all_ids,
                                    _token: '{{ csrf_token() }}'
